class PerlinNoise {
  constructor(seed = Date.now()) {
    ...
    // Duplicate the permutation to remove overflow
    for (let i = 0; i < 512; i++) {
      this.p[i] = this.permutation[i % 256];
    }
  }

  // Hàm fade để làm mượt đường nội suy
  fade(t) {
  ...
  noise(x, y, z) {
    ...
    // Tìm độ lệch trong khối
    x -= Math.floor(x);
    y -= Math.floor(y);
    z -= Math.floor(z);

    // Tính toán các hàm fade
    const u = this.fade(x);
    const v = this.fade(y);
    const w = this.fade(z);

    // Hash các đỉnh của khối
    const A = this.p[X] + Y,
      AA = this.p[A] + Z,
      AB = this.p[A + 1] + Z;
  ...
  // Sinh noise 2D (đơn giản hóa từ 3D)
  noise2D(x, y) {
...
console.log(perlin); // Sinh giá trị noise
